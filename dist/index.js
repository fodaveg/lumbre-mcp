...
server.registerTool('list_tasks', {
    description: 'Lee tareas de Lumbre. `scope`: today (default), week, inbox/someday, overdue, all ' +
    ...
    try {
        ...
        if (notesMode === 'full') {
            ...
            await recordNotesSeen(tasks
                .filter(hasNotes)
                .map((t) => ({ taskId: t.id, notes: t.notes, notesUpdatedAt: t.notesUpdatedAt })));
        }
    ...
    try {
        ...
        if (hasNotes(task)) {
            await recordNotesSeen([
                { taskId: task.id, notes: task.notes, notesUpdatedAt: task.notesUpdatedAt }
            ]);
        }
