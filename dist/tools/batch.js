...
import { errorResult, exposedRecurrenceSchema, formatOpShapeError, formatOutcomeReport, NOTES_OVERWRITE_CAVEAT, OUTCOME_NOTE, recurrencePatchSchema, recurrenceSchema, subtasksSchema, tagSchema, textResult } from './shared.js';
/**
 ...
 */
async function runOpsBatch(ctx, rawOps, strictOpSchema, toolName) {
...
export function registerBatchTool(server, ctx) {
    const mutateTasksTool = server.registerTool('mutate_tasks', {
        ...
        try {
            return textResult(await runOpsBatch(ctx, input.ops, mutateTasksStrictOpSchema, 'mutate_tasks'));
        }
        catch (err) {
    ...
    const organizeTool = server.registerTool('organize', {
        ...
        try {
            return textResult(await runOpsBatch(ctx, input.ops, organizeStrictOpSchema, 'organize'));
        }
        catch (err) {
            return errorResult(err);
        }
