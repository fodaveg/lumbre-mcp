...
function formatTask(t, opts, isDuplicateTitle) {
    ...
    if (t.date && t.time)
        metadata.push(`${t.date} ${t.time}`);
    else if (t.date)
        metadata.push(t.date);
    else if (t.time)
    ...
    if (isDuplicateTitle)
        metadata.push(`creada:${t.createdAt.slice(0, 16)}`);
 ...
 *  resto del módulo. */
function listNotesUpdatedAtIso(notesUpdatedAt) {
    if (typeof notesUpdatedAt !== 'number' || !Number.isFinite(notesUpdatedAt))
        return null;
 ...
 */
function listNoteMarkerSuffix(l) {
...
export function formatListLinks(listId, links) {
    if (links.length === 0)
        return `Sin vínculos para el proyecto o área ${listId}.`;
 ...
 */
export function formatHabitList(habits, habitLog, includeArchived) {
    const visible = includeArchived ? habits : habits.filter((h) => h.archivedAt === undefined);
    const omitted = habits.length - visible.length;
    const omittedSuffix = omitted > 0 ? ` (${omitted} archivado${omitted === 1 ? '' : 's'} omitido${omitted === 1 ? '' : 's'})` : '';
    if (visible.length === 0) {
        return `0 hábitos${omittedSuffix}.${omitted > 0 ? ' includeArchived:true para verlos.' : ''}`;
    }
    const logByHabit = new Map();
    for (const e of habitLog) {
        const list = logByHabit.get(e.habitId);
        if (list)
            list.push(e);
        else
            logByHabit.set(e.habitId, [e]);
    }
    const lines = visible.map((h) => {
        const archived = h.archivedAt !== undefined ? ` [archivado ${new Date(h.archivedAt).toISOString().slice(0, 10)}]` : '';
