...
describe('POST /mcp — con token, contra el servidor real (createServer de index.ts, M1)', () => {
	...
	it('tools/list responde con las 19 tools de producción, sin `$schema` y bajo el mismo techo de bytes que index.test.ts', async () => {
		const res = await fetch(`${baseUrl}/mcp`, {
			method: 'POST',
			headers: { ...JSON_RPC_HEADERS, authorization: 'Bearer tok-válido' },
		...
		// Bajado a 19 tools/23.300 el 2026-08-27 (podadas create_list/
		// nest_list/rename_list/remove_list/move_to_list, mergeadas add/
		// update/delete_brl_entry en mutate_brl — ver `index.test.ts`).
		expect(JSON.stringify(body.result.tools).length).toBeLessThan(23300);
	});
...
describe('POST /mcp — flush automático de sync tras una mutación DE ESTE MCP (marca por token, sync-flush.ts)', () => {
	/**
	 * MISMA mina, MISMO patrón de test que el describe de la caché de
	 * existencia de arriba, y por el MISMO motivo (leer su JSDoc de cabecera):
	 * la marca "mutación pendiente" vive en un registro de MÓDULO justo
	 * porque `handleMcpRequest` llama a `createServer` DENTRO de cada `POST`.
	 * Un test con UN `McpServer` in-memory (una sola llamada a `createServer`)
	 * pasaría en verde con el bug intacto — este cruza dos `fetch` HTTP reales
	 * contra el `server.listen(0)` de `beforeAll`, una mutación en un POST y
	 * la lectura en OTRO, exactamente donde `createServer` se invoca dos
	 * veces.
	 */
	const originalFetch = globalThis.fetch;
	...
	/** Cuenta los `POST /api/sync/flush` (el flush que dispara `refreshSync`,
	 *  a mano vía `refresh_sync` o solo vía `autoFlushSyncIfPending`) —
	 *  separado de la llamada del propio test contra el server HTTP local. */
	function countFlushPosts(fetchSpy: ReturnType<typeof vi.fn>): number {
		return fetchSpy.mock.calls.filter((call) => String(call[0]).includes('/api/sync/flush')).length;
	}
	...
	function stubUpstreamFetch(): ReturnType<typeof vi.fn> {
		const fetchSpy = vi.fn(async (url: string | URL, init?: RequestInit) => {
			const u = String(url);
			if (u.startsWith(baseUrl)) return originalFetch(url, init); // el propio server HTTP local
			if (u.includes('/api/ingest')) return jsonResponse({ ok: true });
			if (u.includes('/api/tasks')) return jsonResponse([]); // list_tasks: sin tareas, basta para el test
			if (u.includes('/api/sync/flush')) return jsonResponse({ ok: true });
			throw new Error(`fetch no mockeado en este test: ${u}`);
		});
		vi.stubGlobal('fetch', fetchSpy);
		return fetchSpy;
	...
	it('add_task (mutación) en un POST, list_tasks (lectura) en OTRO POST, mismo token: la lectura flushea sola', async () => {
		const fetchSpy = stubUpstreamFetch();
		const token = 'tok-flush-mismo';

		// add_task NO flushea por sí sola — solo MARCA que hay algo pendiente
		// para la próxima lectura (ver `markMutationPending` en index.ts).
		const mutation = await toolCall(token, 301, 'add_task', { text: 'prueba flush automático' });
		expect(mutation.isError).not.toBe(true);
		expect(countFlushPosts(fetchSpy)).toBe(0);

		// PETICIÓN HTTP DISTINTA, mismo token: si la marca viviera en la
		// clausura de `createServer` (el bug de esta mañana), esta lectura NUNCA
		// vería que hubo una mutación y `countFlushPosts` seguiría en 0.
		const read = await toolCall(token, 302, 'list_tasks', {});
		expect(read.isError).not.toBe(true);
		expect(countFlushPosts(fetchSpy)).toBe(1);

		// La marca ya se limpió (el flush tuvo éxito): una SEGUNDA lectura, sin
		// mutación de por medio, no vuelve a flushear.
		const readAgain = await toolCall(token, 303, 'list_tasks', {});
		expect(readAgain.isError).not.toBe(true);
		expect(countFlushPosts(fetchSpy)).toBe(1);
	});

	it('aislamiento: la marca de un token no la ve otro token', async () => {
		const fetchSpy = stubUpstreamFetch();

		const mutation = await toolCall('tok-flush-a', 401, 'add_task', { text: 'prueba flush aislado' });
		expect(mutation.isError).not.toBe(true);

		// Lectura con TOKEN DISTINTO: no debe heredar la marca de "tok-flush-a".
		const readOtherToken = await toolCall('tok-flush-b', 402, 'list_tasks', {});
		expect(readOtherToken.isError).not.toBe(true);
		expect(countFlushPosts(fetchSpy)).toBe(0);

		// Lectura con el TOKEN que SÍ mutó: esa sí tenía algo pendiente.
		const readSameToken = await toolCall('tok-flush-a', 403, 'list_tasks', {});
		expect(readSameToken.isError).not.toBe(true);
		expect(countFlushPosts(fetchSpy)).toBe(1);
	});
});
