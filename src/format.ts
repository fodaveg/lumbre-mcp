...
function stateBox(t: LumbreTask): string {
	if (t.cancelledAt) return '[-]';
	return t.done ? '[x]' : '[ ]';
}

/** Estado en palabras para `get_task`, con la fecha de cancelación. */
function stateLabel(t: LumbreTask): string {
	if (t.cancelledAt) return `cancelada (${t.cancelledAt})`;
	return t.done ? 'hecha' : 'pendiente';
}

const FREQ_LABEL: Record<IngestRecurrence['freq'], string> = {
	daily: 'diaria',
	weekly: 'semanal',
	monthly: 'mensual',
	yearly: 'anual'
};
const FREQ_UNIT_PLURAL: Record<IngestRecurrence['freq'], string> = {
	daily: 'días',
	weekly: 'semanas',
	monthly: 'meses',
	yearly: 'años'
 ...
 */
export function formatRecurrence(r: IngestRecurrence): string {
	...
	if (r.mode === 'afterCompletion') parts.push('tras completar');
	if (r.until) parts.push(`hasta ${r.until}`);
	if (r.count !== undefined) parts.push(`${r.count} veces`);
	let label = parts.join(' ');
	if (r.streak) label += ', hábito';
	return label;
}
 ...
 */
function seriesMetadata(t: LumbreTask): string[] {
	const out: string[] = [];
	if (t.recurrence) out.push(`↻${formatRecurrence(t.recurrence)}`);
	if (t.seriesId) out.push(t.seriesId === t.id ? 'semilla' : `serie:${t.seriesId}`);
	return out;
}
...
function formatTask(t: LumbreTask, opts: FormatTaskOptions, isDuplicateTitle: boolean): string {
	...
	if (t.cancelledAt) metadata.push('cancelada');
...
export function formatTaskFull(t: LumbreTask, refs?: RefResolution): string {
	...
	if (t.seriesId) {
		lines.push(
			`- serie: ${t.seriesId === t.id ? 'semilla de su serie' : `ocurrencia; semilla ${t.seriesId}`}`
		);
	}
