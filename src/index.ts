...
export function createServer(config: LumbreConfig, opts: CreateServerOptions = {}): McpServer {
	...
	const getListLinksTool = server.registerTool(
		...
		{
			description:
				'Lee las notas vinculadas de UNA lista por su listId (incluye URL y metadata; puede ser ' +
	...
	async function requireBrlEntryExists(date: string, entryId: string): Promise<void> {
		...
		if (brlCache.has(date, entryId)) return;
	 ...
	 */
	const mutateBrlTool = server.registerTool(
		'mutate_brl',
		{
			description:
				`Vía PREFERENTE (y desde el 2026-08-27, ÚNICA — sustituye a add/update/delete_brl_entry) ` +
