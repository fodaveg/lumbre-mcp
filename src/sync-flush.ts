...
const registryByToken = new Map<string, TokenFlushState>();

function pruneIdleTokens(nowMs: number): void {
	for (const [token, state] of registryByToken) {
		if (nowMs - state.lastAccessMs > IDLE_EVICT_MS) registryByToken.delete(token);
	}
}
...
function getState(token: string, nowMs: number): TokenFlushState {
 ...
 * bien, no si todas.
 */
export function markMutationPending(token: string, now: () => number = Date.now): void {
	getState(token, now()).pending = true;
}

/** `true` si `token` tiene una mutación pendiente de flush. */
export function isMutationPending(token: string, now: () => number = Date.now): boolean {
...
export function clearMutationPending(token: string, now: () => number = Date.now): void {
 ...
 */
export function resetSyncFlushRegistryForTests(): void {
