...
function normalizeStore(value: unknown): OAuthStore {
	...
	const raw = value as { version?: unknown; grants?: unknown; usedRefreshTokens?: unknown };
	if ((raw.version !== 1 && raw.version !== 2) || !Array.isArray(raw.grants)) {
		throw new Error('store OAuth inválido');
	}
	const grants = raw.grants.map((value, index): StoredGrant => {
		...
		const familyId =
			raw.version === 2 && typeof grant.familyId === 'string'
				? grant.familyId
				: digest(`familia:${index}:${grant.accessHash}:${grant.refreshHash}`);
		const familyExpiresAt =
			typeof grant.familyExpiresAt === 'number' && Number.isFinite(grant.familyExpiresAt)
				? grant.familyExpiresAt
				: (grant.refreshExpiresAt as number);
		return { ...(grant as StoredGrant), familyId, familyExpiresAt };
	});
	const usedRefreshTokens =
		raw.version === 2 && Array.isArray(raw.usedRefreshTokens)
			? raw.usedRefreshTokens.map((value): UsedRefreshToken => {
...
export class OAuthService {
	...
	private readonly transactions = new Map<string, AuthorizationRequest & { expiresAt: number; clientName: string }>();
	...
	async handleAuthorize(req: IncomingMessage, res: ServerResponse, url: URL, upstreamBaseUrl: string): Promise<void> {
...
<button type="submit">Autorizar</button>
			...
			if (!transaction || transaction.expiresAt <= this.now()) throw new OAuthError('invalid_request', 'La autorización ha caducado; vuelve a iniciarla.');
			if (!upstreamToken || upstreamToken.length > 512) throw new OAuthError('access_denied', 'Token de Lumbre inválido.');
			...
			if (transaction.state !== undefined) redirect.searchParams.set('state', transaction.state);
	...
	private async validateUpstreamToken(baseUrl: string, token: string): Promise<void> {
		let response: Response;
		try {
		...
		if (status === 401 || status === 403) throw new OAuthError('access_denied', 'El token de Lumbre no es válido.', 401);
		if (status < 200 || status >= 300) throw new OAuthError('temporarily_unavailable', 'Lumbre no pudo validar el token.', 503);
	}
	...
	private async exchangeCode(form: URLSearchParams, res: ServerResponse): Promise<void> {
		...
		if (!code || code.expiresAt <= this.now()) throw new OAuthError('invalid_grant', 'Código inválido, usado o caducado.');
		...
		if (!verifier || !/^[A-Za-z0-9._~-]{43,128}$/.test(verifier)) throw new OAuthError('invalid_grant', 'code_verifier inválido.');
		const actual = createHash('sha256').update(verifier, 'ascii').digest('base64url');
		if (!equalText(actual, code.challenge)) throw new OAuthError('invalid_grant', 'PKCE no coincide.');
		await this.issueGrant(code, res);
	}

	private async issueGrant(code: PendingCode, res: ServerResponse): Promise<void> {
	...
	private async exchangeRefresh(form: URLSearchParams, res: ServerResponse): Promise<void> {
		...
		if (result.outcome !== 'rotated') throw new OAuthError('invalid_grant', 'Refresh token inválido, usado o caducado.');
		...
		try {
			...
			await this.mutateStore((store) => {
				let familyId = store.grants.find(
					(grant) => grant.clientId === clientId && (grant.accessHash === hash || grant.refreshHash === hash)
				)?.familyId;
				familyId ??= store.usedRefreshTokens.find((item) => item.hash === hash)?.familyId;
				if (!familyId) return false;
				const familyGrant = store.grants.find(
					(grant) => grant.familyId === familyId && grant.clientId === clientId
				);
	...
	async resolveAccessToken(token: string): Promise<string | undefined> {
		...
		try {
			...
			if (!grant || grant.accessExpiresAt <= now || grant.resource !== OAUTH_RESOURCE || grant.scope !== OAUTH_SCOPE) return undefined;
	...
	async ensureReady(): Promise<void> {
		...
		for (const grant of store.grants) {
			...
			if (upstream.length === 0 || upstream.length > 512) throw new Error('credencial upstream inválida');
		}
	...
	private prunePending(): void {
		...
		for (const [id, code] of this.codes) {
			if (code.expiresAt <= now) this.codes.delete(id);
		}
		while (this.transactions.size >= MAX_PENDING_ITEMS) this.transactions.delete(this.transactions.keys().next().value!);
		while (this.codes.size >= MAX_PENDING_ITEMS) this.codes.delete(this.codes.keys().next().value!);
	}
