...
/**
 * Estado de una op aceptada, tal como lo cuenta el informe al modelo: los
 * cuatro de `/api/batch` (`DrainItemStatus`), los dos que solo tiene
 * `/api/mutations` (`not-found`, `queued`) y `unconfirmed` para un servidor
 * que no manda el campo.
 */
...
export function formatOutcomeReport(entries: OpOutcomeEntry[], notices: string[]): string {
	const lines: string[] = [];
	if (entries.length > 0) {
		const counts = new Map<OpOutcome, number>();
		for (const e of entries) counts.set(e.outcome, (counts.get(e.outcome) ?? 0) + 1);
		const parts = (Object.keys(OUTCOME_COUNT_LABEL) as OpOutcome[])
