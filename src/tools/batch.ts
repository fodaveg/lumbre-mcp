...
import { ASYNC_NOTE, errorResult, formatOpShapeError, recurrenceSchema, tagSchema, textResult, type ToolCtx } from './shared.js';
		...
		async (input) => {
			try {
				...
				if (!plan.split) {
					...
					const phase1Results =
						mutatePhase.ops.length > 0 ? await runBatch(ctx.config, mutatePhase.ops) : [];
					...
					const phase2Results =
						phase2.ops.length > 0 ? await runBatch(ctx.config, phase2.ops) : [];
